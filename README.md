...
A custom-built Splunk SIEM dashboard designed to monitor, visualize, and analyze login activity, detect failed login attempts, and identify suspicious or unauthorized access patterns in real time. Developed using Splunk's Search Processing Language (SPL) to query, correlate, and present security log data through interactive dashboard panels.
## 🛠️ Features
